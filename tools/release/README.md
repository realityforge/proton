...
This workflow publishes Proton to Maven Central using the split release scripts in this directory. Run commands from the
repository root.
...
* Maven Central username defaults to the legacy user `realityforge` and can be overridden with
...
The package command writes `dist/proton-<version>/` and `dist/proton-<version>.zip`.
