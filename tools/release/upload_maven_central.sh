...
BUNDLE="dist/proton-${VERSION}.zip"
if [[ ! -f "${BUNDLE}" ]]; then
...
MAVEN_CENTRAL_USERNAME="${MAVEN_CENTRAL_USERNAME:-realityforge}"
...
curl --fail-with-body \
  ...
  -F "bundle=@${BUNDLE}" \
  -o "${UPLOAD_RESPONSE}" \
  "${BASE_URL}/api/v1/publisher/upload?name=proton-${VERSION}&publishingType=AUTOMATIC"
