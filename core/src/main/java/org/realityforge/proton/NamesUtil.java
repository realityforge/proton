...
public final class NamesUtil
{
  ...
  @Nonnull
  public static String mustBeJavaIdentifier( @Nonnull final String annotationClassname,
                                             ...
                                             @Nonnull final Element element )
  {
    if ( !SourceVersion.isIdentifier( name ) )
    {
      throw new ProcessorException( MemberChecks.toSimpleName( annotationClassname ) + " target specified an " +
                                    "invalid value '" + name + "' for the parameter " + parameterName + ". " +
                                    "The value must be a valid java identifier", element );
    }
    else if ( SourceVersion.isKeyword( name ) )
    {
      throw new ProcessorException( MemberChecks.toSimpleName( annotationClassname ) + " target specified an " +
                                    "invalid value '" + name + "' for the parameter " + parameterName + ". " +
                                    "The value must not be a java keyword", element );
    }
    else
    {
      return name;
  ...
  @Nullable
  public static String deriveName( @Nonnull final Element element,
  ...
  @Nonnull
  public static String getPropertyAccessorName( @Nonnull final ExecutableElement method,
    ...
    if ( sentinelValue.equals( declaredName ) )
    {
      ...
      if ( null == defaultValue )
      {
        throw new ProcessorException( MemberChecks.toSimpleName( annotationClassname ) + " target did not specify " +
                                      "the parameter " + parameterName + " and the default value could not be " +
                                      "derived",
                                      method );
      }
      return defaultValue;
