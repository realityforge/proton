...
{
  @Test
  public void elementHelpersTraverseTypeHierarchiesAndElementMetadata()
    throws Exception
  {
    final ElementProcessor processor = new ElementProcessor();

    TestUtil.compile( List.of( TestUtil.source( "com.example.ElementsTarget", """
      package com.example;
      import java.util.List;
  ...
  private static final class ElementProcessor
    extends TestUtil.TestProcessor
  {
    ...
    private void validateHierarchyHelpers( @Nonnull final TypeElement target, @Nonnull final TypeElement base )
    {
      ...
      assertEquals( fields.stream().map( f -> f.getSimpleName().toString() ).collect( Collectors.toList() ),
      ...
      final Map<String, List<ExecutableElement>> methodsByName =
        ElementsUtil
          .getMethods( target, processingEnv.getElementUtils(), processingEnv.getTypeUtils() )
          .stream()
      ...
      assertTrue( ElementsUtil.doesMethodOverrideInterfaceMethod( processingEnv.getTypeUtils(),
                                                                  target,
                                                                  method( target, "root" ) ) );
      assertFalse( ElementsUtil.doesMethodOverrideInterfaceMethod( processingEnv.getTypeUtils(),
                                                                   target,
                                                                   method( target, "convert" ) ) );
      assertEquals( ElementsUtil.toRawType( field( target, "names" ).asType() ).toString(), "java.util.List" );
      assertSame( ElementsUtil.getOverriddenMethod( processingEnv, target, method( target, "convert" ) ),
                  method( base, "convert" ) );
      assertNull( ElementsUtil.getOverriddenMethod( processingEnv, target, method( target, "own" ) ) );
    }

    private void validateElementMetadataHelpers( @Nonnull final TypeElement target,
                                                 ...
                                                 @Nonnull final TypeElement otherType )
    {
      ...
      assertEquals( ElementsUtil.getPackageElement( inner ).getQualifiedName().toString(), "com.example" );
