...
public final class AnnotationsUtilTest
{
  ...
  @Test
  public void annotationHelpersReadMirrorsValuesAndDefaults()
    throws Exception
  {
    ...
    extends TestUtil.TestProcessor
  {
    ...
    public boolean process( final Set<? extends TypeElement> annotations, final javax.annotation.processing.RoundEnvironment roundEnv )
    {
      ...
      {
        final TypeElement target = processingEnv.getElementUtils().getTypeElement( "com.example.AnnotationTarget" );
    ...
    private void validateTypeAnnotations( @Nonnull final TypeElement target )
    {
      ...
      assertEquals( AnnotationsUtil.getAnnotationValue( target, MARKER, "name" ).getValue(), "explicit" );
      assertEquals( AnnotationsUtil.findAnnotationValue( target, MARKER, "name" ).getValue(), "explicit" );
      assertEquals( AnnotationsUtil.findAnnotationValueNoDefaults( marker, "name" ).getValue(), "explicit" );
      assertEquals( AnnotationsUtil.getEnumAnnotationParameter( target, MARKER, "mode" ), "SLOW" );

      final List<String> typeNames =
        AnnotationsUtil.getTypeMirrorsAnnotationParameter( target, MARKER, "types" )
          .stream()
          .map( Object::toString )
    ...
    private void validateFieldAnnotations( @Nonnull final TypeElement target )
    {
      ...
      final Map<ExecutableElement, AnnotationValue> values = AnnotationsUtil.getAnnotationValuesWithDefaults( marker );
      assertEquals( values.keySet().stream().map( e -> e.getSimpleName().toString() ).collect( Collectors.toList() ),
    ...
    {
      assertEquals( tagValues( AnnotationsUtil.getRepeatingAnnotations( method( target, "singleTag" ), TAGS, TAG ) ),
                    List.of( "single" ) );

      final Function<ExecutableElement, String> deriveName = method -> method.getSimpleName() + "Name";
      assertEquals( AnnotationsUtil.extractName( method( target, "named" ),
                                                 deriveName,
                                                 NAMING,
                                                 "value",
                                                 "<default>" ),
                    "renamed" );
      assertEquals( AnnotationsUtil.extractName( method( target, "derived" ),
                                                 deriveName,
                                                 NAMING,
                                                 "value",
                                                 "<default>" ),
      ...
      assertProcessorException( () -> AnnotationsUtil.extractName( method( target, "missingDefault" ),
                                                                   method -> null,
                                                                   NAMING,
                                                                   "value",
                                                                   "<default>" ),
                                "@Naming target did not specify the parameter value and the default value could not be derived" );
      assertProcessorException( () -> AnnotationsUtil.extractName( method( target, "invalidIdentifier" ),
                                                                   deriveName,
                                                                   NAMING,
                                                                   "value",
                                                                   "<default>" ),
      ...
      assertProcessorException( () -> AnnotationsUtil.extractName( method( target, "keywordIdentifier" ),
                                                                   deriveName,
                                                                   NAMING,
                                                                   "value",
                                                                   "<default>" ),
    ...
    private static void assertProcessorException( @Nonnull final ThrowingRunnable runnable,
