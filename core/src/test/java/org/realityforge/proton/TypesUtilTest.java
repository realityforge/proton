...
{
  @Test
  public void typeInspectionHelpersDetectArraysRawTypesWildcardsAndDeprecation()
    throws Exception
  {
    ...
    extends TestUtil.TestProcessor
  {
    ...
    public boolean process( final Set<? extends TypeElement> annotations, final javax.annotation.processing.RoundEnvironment roundEnv )
    {
      if ( !_validated && !roundEnv.processingOver() )
      {
        ...
        assertTrue( TypesUtil.hasRawTypes( processingEnv, fields.get( "rawList" ).asType() ) );
        assertTrue( TypesUtil.hasRawTypes( processingEnv, fields.get( "nestedRawList" ).asType() ) );
        assertFalse( TypesUtil.hasRawTypes( processingEnv, fields.get( "typedList" ).asType() ) );

        assertTrue( TypesUtil.isDeprecated( processingEnv, fields.get( "old" ).asType() ) );
        assertTrue( TypesUtil.isDeprecated( processingEnv, fields.get( "oldList" ).asType() ) );
        assertFalse( TypesUtil.isDeprecated( processingEnv, fields.get( "string" ).asType() ) );
    ...
    @Nonnull
    private ExecutableType executableType( @Nonnull final DeclaredType targetType,
